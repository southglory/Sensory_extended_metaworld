'''
Normalize ours obj files to [-0.5, 0.5]^3.
...
def normalize(obj_path):
    '''
    normalize shapes
    :param obj_path: Ours object path
    :return:
    '''
    cat, obj_file = obj_path.split('/')[3:5]

    input_cat_dir = append_dir(os.path.join(ours_path, cat), obj_file, 'i')
    output_cat_dir = append_dir(os.path.join(ours_normalized_path, cat), obj_file, 'o')

    recursive_normalize(input_cat_dir, output_cat_dir)

if __name__ == '__main__':
    if not os.path.exists(ours_normalized_path):
        os.mkdir(ours_normalized_path)

    all_objects = load_data_path(ours_path)
